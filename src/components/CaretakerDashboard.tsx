...
//   const [activeTab, setActiveTab] = useState("overview");
//   const [selectedDate, setSelectedDate] = useState<Date>(new Date());
//   const [medicationToday, setMedicationToday] = useState<any>(null);
...
//   useEffect(() => {
//     const fetchTodayMedication = async () => {
//       const today = format(new Date(), "yyyy-MM-dd");
//       const { data, error } = await supabase
//         .from("meds-buddy-check")
//         .select("*")
//         .eq("date", today);
//       if (!error && data.length > 0) {
//         setMedicationToday(data[0]);
//       }
//     };
...
//                   <div className="flex items-center justify-between p-3 bg-accent/50 rounded-lg">
//                     <div>
//                       <h4 className="font-medium">{medicationToday.name}</h4>
//                       <p className="text-sm text-muted-foreground">{medicationToday.time || "8:00 AM"}</p>
//                     </div>
//                     <Badge variant={medicationToday.status === "completed" ? "secondary" : "destructive"}>
//                       {medicationToday.status === "completed" ? "Completed" : "Pending"}
//                     </Badge>
//                   </div>
//                 ) : (
//                   <p className="text-sm text-muted-foreground">No medication found for today.</p>
//                 )}
//               </CardContent>
//             </Card>
...
//           <Card>
//             <CardHeader><CardTitle>All Medications</CardTitle></CardHeader>
...
//                       </p>
//                     </div>
//                     <Badge variant={med.status === "completed" ? "secondary" : "destructive"}>
//                       {med.status || "Pending"}
//                     </Badge>
//                   </div>
//                 ))
  ...
  useEffect(() => {
    const fetchTodayMedication = async () => {
      ...
      if (!error && data && data.length > 0) {
        const pendingMed = data.find((med) => med.status !== "completed");
        setMedicationToday(pendingMed || data[0]);
      }
    };
